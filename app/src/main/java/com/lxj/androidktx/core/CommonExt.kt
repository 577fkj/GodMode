 ...
 * 将Bitmap保存到相册
 */
fun Bitmap.saveToAlbum(format: Bitmap.CompressFormat = PNG, quality: Int = 100, filename: String = "", callback: ((path: String?, uri: Uri?)->Unit)? = null ) {
    GlobalScope.launch {
        try {
            //1. create path
            ...
            //3. notify
            MediaScannerConnection.scanFile(AndroidConfig.context, arrayOf(target.absolutePath),
